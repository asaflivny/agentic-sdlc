    ...
    *repo* must be in ``owner/name`` form.
    ...
    async with httpx.AsyncClient(headers=headers, timeout=15.0) as client:
        ...
        if pr_number is None:
            logger.info("github: no open PR found for branch=%s repo=%s, skipping comment", branch, repo)
            return
        ...
        if r.status_code in (200, 201):
            logger.info("github: posted PR comment pr=%d repo=%s findings=%d", pr_number, repo, total)
        else:
            logger.warning("github: comment post failed pr=%d status=%d body=%s", pr_number, r.status_code, r.text[:200])
...
def _build_comment(result: "WorkflowResult", total: int) -> str:
    ...
    for agent_result in result.agent_results:
        if not agent_result.findings:
            continue
        lines.append(f"<details><summary><strong>{agent_result.agent_name}</strong> — {len(agent_result.findings)} finding(s)</summary>")
        lines.append("")
        for f in agent_result.findings:
            badge = _SEVERITY_BADGE.get(f.severity, f.severity)
            loc = f" · `{f.file_path}`" + (f":{f.line_number}" if f.line_number else "") if f.file_path else ""
